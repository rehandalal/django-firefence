...
class TestFence(object):
    def test_set_rules(self):
        ...
        ruleset = RuleSet([Rule(action=Rule.DENY, address='127.0.0.1')])
        fence = Fence(ruleset)
        assert len(fence.rules) == 1
        assert fence.rules[0].action == Rule.DENY
        assert fence.rules[0].address == '127.0.0.1'

        # Test that you can pass through an array of rules
        fence = Fence([{'action': Rule.ALLOW, 'address': '192.168.1.1'}])
        assert len(fence.rules) == 1
        assert fence.rules[0].action == Rule.ALLOW
        assert fence.rules[0].address == '192.168.1.1'
